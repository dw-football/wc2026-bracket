...
## RESUME
Next action (resume on **520 / NYWDWARREN2**, where the worktrees + the armed auto-sync live): **continue the KO build** — open Claude in `~/src/wc2026-bracket`, "read CLAUDE.md and continue", build pieces #1→#4 in the **`ko-build` worktree** (`C:\Users\dwarren\src\wc2026-ko`), test on localhost, **merge to main + push before Sun Jun 28 3pm EDT**. See the **BUILD STATUS** block below for exact progress (#1 parser done @ `6b98115`) + the roadmap. Routine group-stage scores are now FULLY AUTOMATED by the armed auto-sync (no human needed) — live currently **54/104** (commit `b9d4257`), groups finish ~Jun 27.
⚠️ If picking up on ANOTHER machine: the `ko-build` + `demo-mid-r32-backup` branches and the latest `CLAUDE.md` may be LOCAL-ONLY on 520 (see the wrap's commit-gate outcome) — `git fetch` and check, or they won't be there.
**#4 unattended auto-sync — DECISIONS MADE + DRY-RUN BUILT (2026-06-24 eve), NOT committed/armed.** David's calls: **(a)** reuse the existing mcp-gsuite OAuth token (CONFIRMED + PROVEN — the cached refresh_token [file path lives in the gitignored `run-autosync.cmd` as `$GSUITE_OAUTH_FILE`; see `~/My Drive/Computing/Claude/integrations.md`] carries `…/auth/calendar`; headless token-mint → Calendar REST verified, the Sports calendar [real ID in the gitignored `calendar-map.local.json`] writable as owner); **(b)** local **Windows Task Scheduler on 520**; **(c)** **fully unattended** (auto-push LIVE site AND calendar). **Polling design (David's):** start checking **KO+115 min**, then **every 5 min** until FT; **simultaneous SET = both games of a group's final matchday — deploy only when BOTH FT** (hold a half-finished set). **Rain-delay doctrine:** clock only gates START; ESPN `status.completed` alone = "done"; HOLD a not-yet-FT sibling while it's legitimately coming (pre/in/delayed/ET), deploy-partial+FLAG only on postponed/abandoned or a 6h backstop (a 2hr rain delay already happened — expect more). **Notify = FAILURE-ONLY** (silent on success so a score is never spoiled — David watches late games on replay in the morning).
Built (localhost, UNPUSHED per workflow rule): **`espn-poll.mjs`** (read-only schedule-from-our-repo + ESPN status, set-batching, classify), **`calendar-apply.mjs`** (headless REST writer, dry-run default, `--apply` to write), **`autosync.mjs`** (orchestrator; dry-run default; LIVE double-gated behind `--arm` + `AUTOSYNC_LIVE=1`; tracks processed sets in gitignored `autosync-state.json`). All three verified dry-run end-to-end (FT-injection → deployable pair; calendar token-mint → GET diff = would-write 21/up-to-date 10/0 errors).
**LIVE STATUS (2026-06-24 ~8:20pm ET):** Dry-run PASSED on Group C (caught Scotland 0-3 Brazil + Morocco 4-2 Haiti, held for both, ESPN-confirmed). **Group C DEPLOYED LIVE** (site commit `5146ab5` + calendar applied, 52/104). **WTS task `WC2026-autosync-dryrun` is now ARMED LIVE on 520** (`run-autosync.cmd` has `AUTOSYNC_LIVE=1` + `--arm`; MultipleInstances=IgnoreNew) — it will auto-deploy the **9pm ET Group A games** (Czech Republic v Mexico, South Africa v South Korea; due 10:55pm ET, ~11pm finish) site+calendar UNATTENDED, silent on success. Claude is background-watching `autosync.log` for that first live deploy to confirm/flag (failure notify is log-only for now). **The auto-sync feature CODE is NOT pushed** (untracked: espn-poll/calendar-apply/autosync.mjs, run-autosync.cmd; CLAUDE.md + .gitignore modified-uncommitted) — per workflow rule, publish only on David's explicit OK. `deployLive` stages ONLY the 3 score files (never `git add -A`).
Optimization added: poller skips the ESPN fetch entirely until a match is past KO+115 (idle ticks are free no-ops). Overnight OFF 4-11am NY.
**Remaining before it's fully hands-off:** (1) **knockout result handling** — poller maps GROUP matches only; R32 starts Jun 28, needs KO score+ET+penalty entry; (2) wire the failure-only **Gmail-REST notify** (same token, mail scope present) so a failed unattended run actively pings David instead of only logging; (3) optional **MC-jitter threshold** so sub-1% sampling drift doesn't rewrite ~20 calendar events every sync; (4) decide whether to PUBLISH the auto-sync feature code; (5) survive-logoff (task runs only while David is logged in on 520). See Session Notes 2026-06-24 (eve).
Open code TODO (Claude-autonomous): guard `build-teams.mjs` so an Elo re-scrape preserves `team.worldRank`. (NOTE: the BIGGER gap — `adapter.js toGroups` was DROPPING `worldRank` entirely, so the step-7 tiebreaker silently used the Elo proxy despite the data+comparator being in place — is **FIXED + LIVE, commit `cdfeeea`** 2026-06-24; toGroups now carries `worldRank`, 74/74 tests green, engine uses real FIFA ranks. The build-teams guard is the remaining smaller risk.)
